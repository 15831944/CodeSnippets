...
namespace CADBest.GeometryNamespace
{
    ...
    public static class Geometry
    {
        ...
        /// <summary>
        /// Mesure angle between two connected triangles
        /// </summary>
        /// <param name="Points">List with Four points</param>
        /// <returns>Angle in radians</returns>
        public static double AngleBetweenTriangles(List<Point3D> Points)
        {
            if (Points.Count != 4)
            {
                return -1;
            }
            List<Point3D> FourPoints = Geometry.CloneList(Points);
            List<Point3D> sourceOrientation = Geometry.align_calc_pars(FourPoints.GetRange(0, 3));
            List<Point3D> originOXYParameters = new List<Point3D>(OriginAlignParametersOXY.ToList<Point3D>());

            Geometry.align(FourPoints, sourceOrientation, originOXYParameters);
            Point3D SinCosP4 = Geometry.calc_rotX(FourPoints[3]);

            return Geometry.CalculateAngle(SinCosP4);
        }

        /// <summary>
        /// Choose better situation of creating two pairs of triangles. Based on angle between them.
        /// </summary>
        /// <param name="Points">Four Point</param>
        /// <returns></returns>
        public static int BestTriangle(List<Point3D> Points)
        {
            List<Point3D> FourPoints = Geometry.CloneList(Points);

            double FirstResult = AngleBetweenTriangles(FourPoints);

            FourPoints.Add(FourPoints[0]);
            FourPoints.RemoveAt(0);

            double SecondResult = AngleBetweenTriangles(FourPoints);

            if (FirstResult > Math.PI)
                FirstResult = 2 * Math.PI - FirstResult;

            if (SecondResult > Math.PI)
                SecondResult = 2 * Math.PI - SecondResult;
        ...
        /// </returns>
        private static List<Point3D> homography_calc_additional_pars(Point3D P0, Point3D P1, Point3D P2, Point3D P3)
        {
